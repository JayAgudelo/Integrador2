...
if __name__ == "__main__":
    audio_file = "backend/api/test_song.mp3"
    
    print("Getting complete feature set...")
    features = get_complete_features(audio_file)
    
    if features:
        print("\n✓ Successfully extracted ALL features!\n")
        
        # Reorder columns to match your model's expected order
        column_order = [
            'acousticness', 'danceability', 'energy', 'instrumentalness', 
            'key', 'liveness', 'loudness', 'mode', 'speechiness', 'tempo', 
            'time_signature', 'valence', 'duration_ms', 'genre'
        ]
        ...
        # Save to CSV
        df.to_csv("complete_audio_features.csv", index=False)
